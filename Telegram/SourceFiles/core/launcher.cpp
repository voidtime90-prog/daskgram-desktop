...
bool MoveLegacyAlphaFolder(const QString &folder, const QString &file) {
	...
	const auto now = cExeDir() + u"DaskGramForcePortable"_q;
...
bool MoveLegacyAlphaFolder() {
	if (!MoveLegacyAlphaFolder(u"DaskGramAlpha_data"_q, u"alpha"_q)
		|| !MoveLegacyAlphaFolder(u"DaskGramBeta_data"_q, u"beta"_q)) {
		return false;
	}
	return true;
}
...
bool CheckPortableVersionFolder() {
	...
	const auto portable = cExeDir() + u"DaskGramForcePortable"_q;
...
void Launcher::init() {
	...
	QApplication::setApplicationName(u"DaskGram"_q);
...
int Launcher::exec() {
	...
	// chosen: a translocated bundle never sees its DaskGramForcePortable.
	...
	DEBUG_LOG(("DaskGram finished, result: %1").arg(result));
	...
	if (!UpdaterDisabled() && cRestartingUpdate()) {
		...
		DEBUG_LOG(("Sandbox Info: executing DaskGram because of restart."));
