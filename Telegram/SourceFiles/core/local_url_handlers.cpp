	...
	if (domainParam == u"oauth"_q) {
		...
		if (!token.isEmpty()) {
			UrlAuthBox::ActivateUrl(
				controller->uiShow(),
				&controller->session(),
				u"dg://resolve?domain=oauth&startapp="_q
...
bool ResolveOAuth(
	...
	UrlAuthBox::ActivateUrl(
		controller->uiShow(),
		&controller->session(),
		u"dg://oauth?token="_q + qthelp::url_encode(token),
		context);
	return true;
}
...
QString TryConvertUrlToLocal(QString url) {
	...
	if (telegramMeMatch) {
		...
		if (const auto phoneMatch = regex_match(u"^\\+([0-9]+)(\\?|$)"_q, query, matchOptions)) {
			...
			return u"dg://addstyle?slug="_q + url_encode(addStyleMatch->captured(1));
			...
			const auto type = regex_match(u"^[a-fA-F0-9]{6}^"_q, bg)
				...
				? "gradient"
				: "slug";
			return u"dg://bg?"_q + type + '=' + bg + (params.isEmpty() ? QString() : '&' + params);
			...
			auto result = u"dg://newbot?manager="_q + url_encode(manager);
			if (!username.isEmpty()) {
			...
			if (params.indexOf("boost", 0, Qt::CaseInsensitive) >= 0
				...
				return u"dg://boost?channel="_q + channel;
			}
			const auto base = u"dg://privatepost?channel="_q + channel;
			auto added = QString();
			...
			if (params.indexOf("boost", 0, Qt::CaseInsensitive) >= 0
				&& params.toLower().split('&').contains(u"boost"_q)) {
				return u"dg://boost?domain="_q + domain;
			} else if (domain == u"boost"_q) {
				if (const auto domainMatch = regex_match(u"^/([a-zA-Z0-9\\.\\_]+)(/?\\?|/?$)"_q, usernameMatch->captured(2))) {
					return u"dg://boost?domain="_q + domainMatch->captured(1);
				} else if (params.indexOf("c=", 0, Qt::CaseInsensitive) >= 0) {
					return u"dg://boost?"_q + params;
				}
			}
			const auto base = u"dg://resolve?domain="_q + url_encode(usernameMatch->captured(1));
			auto added = QString();
...
bool IsMiniAppUrl(const QString &url) {
	...
	const auto prefix = u"dg://resolve?"_q;
...
[[nodiscard]] InternalLinkCheckResult InternalLinkCheck(const QString &url) {
	...
	if (!urlTrimmed.startsWith(u"dg://"_q, Qt::CaseInsensitive)) {
		return {};
	}
	const auto command = base::StringViewMid(urlTrimmed, u"dg://"_q.size());
