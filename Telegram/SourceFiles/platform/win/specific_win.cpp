...
#ifdef OS_WIN_STORE
		return appData.absolutePath() + u"/DaskGram UWP/"_q;
#else // OS_WIN_STORE
...
void psDoFixPrevious() {
	try {
		...
		if (SUCCEEDED(userDesktopRes)) {
			userDesktopLnk = QString::fromWCharArray(userDesktopFolder) + "\\DaskGram.lnk";
		}
		if (SUCCEEDED(commonDesktopRes)) {
			commonDesktopLnk = QString::fromWCharArray(commonDesktopFolder) + "\\DaskGram.lnk";
		}
...
void AutostartToggle(bool enabled, Fn<void(bool)> done) {
	...
	const auto success = ManageAppLink(
		...
		FOLDERID_Startup,
		L"-autostart",
		L"DaskGram autorun link.\n"
		"You can disable autorun in DaskGram settings.");
	if (done) {
...
void psSendToMenu(bool send, bool silent) {
	ManageAppLink(
		...
		FOLDERID_SendTo,
		L"--",
		L"DaskGram send to link.\n"
		"You can disable send to menu item in DaskGram settings.");
}
